...
@patch('app.services.component_service.PropertyService')
def test_should_validate_properties_successfully_when_creating_component_with_valid_properties(mock_property_service, component_service, app_context):
    """
    Test: Component creation with valid property data
    Given: Component data with valid predefined properties
    When: create_component is called with property validation
    Then: Properties should be validated and component created successfully
    """
    with patch('app.services.component_service.db.session') as mock_db_session, \
         patch('app.services.component_service.Component') as mock_component, \
         patch('app.services.component_service.ComponentService._check_duplicate_component') as mock_check_duplicate, \
         patch('app.services.component_service.ComponentService._handle_component_associations') as mock_handle_associations:
        
        # Mock successful property validation
        mock_property_service.validate_property_values.return_value = {
            'valid': True,
            'errors': []
        }
        
        # Mock no duplicate component
        mock_check_duplicate.return_value = None
        
        # Mock component creation
        mock_component_instance = Mock()
        mock_component_instance.id = 1
        ...
        # Verify property validation was called
        mock_property_service.validate_property_values.assert_called_once_with(
            1, 
            {'material': 'Cotton', 'color': 'Red', 'custom_notes': 'Test notes'}
        )
        
        # Verify component was created with properties
        mock_component.assert_called_once()
        call_args = mock_component.call_args[1]
        assert call_args['properties'] == {'material': 'Cotton', 'color': 'Red', 'custom_notes': 'Test notes'}
        assert call_args['component_type_id'] == 1
...
@patch('app.services.component_service.PropertyService')
def test_should_fail_creation_when_property_validation_fails(mock_property_service, component_service, app_context):
    """
    Test: Component creation fails with invalid property data
    Given: Component data with invalid predefined properties
    When: create_component is called with invalid properties
    Then: ValueError should be raised with property validation errors
    """
    with patch('app.services.component_service.ComponentService._check_duplicate_component') as mock_check_duplicate:
        
        # Mock property validation failure
        mock_property_service.validate_property_values.return_value = {
            'valid': False,
            'errors': ['Material must be one of: Cotton, Polyester', 'Color is required']
        }
        ...
        assert 'Material must be one of: Cotton, Polyester' in str(exc_info.value)
        assert 'Color is required' in str(exc_info.value)


@patch('app.services.component_service.PropertyService')
def test_should_parse_json_properties_correctly_when_string_provided_during_creation(mock_property_service, component_service, app_context):
    """
    Test: Component creation with JSON string properties
    Given: Component data with properties as JSON string
    When: create_component is called
    Then: Properties should be parsed correctly and validated
    """
    with patch('app.services.component_service.db.session') as mock_db_session, \
         patch('app.services.component_service.Component') as mock_component, \
         patch('app.services.component_service.ComponentService._check_duplicate_component') as mock_check_duplicate, \
        ...
        component_data = {
            'product_number': 'SHIRT-001',
            'component_type_id': 1,
            'properties': '{"material": "Cotton", "color": "Red", "custom_notes": "Test notes"}'
        }
        
        result = component_service.create_component(component_data)
        
        # Verify property validation was called with parsed JSON
        mock_property_service.validate_property_values.assert_called_once_with(
            1, 
            {'material': 'Cotton', 'color': 'Red', 'custom_notes': 'Test notes'}
        )
        
        # Verify component was created with parsed properties
        mock_component.assert_called_once()
        call_args = mock_component.call_args[1]
        assert call_args['properties'] == {'material': 'Cotton', 'color': 'Red', 'custom_notes': 'Test notes'}
...
@patch('app.services.component_service.PropertyService')
def test_should_validate_properties_successfully_when_updating_component(mock_property_service, component_service, app_context):
    """
    Test: Component update with valid property data
    Given: Existing component and valid property updates
    When: update_component is called with property changes
    Then: Properties should be validated and component updated successfully
    """
    with patch('app.services.component_service.Component') as mock_component:
        
        # Mock existing component
        mock_component_instance = Mock()
        mock_component_instance.id = 1
        mock_component_instance.component_type_id = 1
        mock_component_instance.properties = {'material': 'Cotton', 'color': 'Red'}
        mock_component.query.get.return_value = mock_component_instance
        ...
        changes = component_service_instance._update_basic_fields(mock_component_instance, update_data)
        
        # Verify property validation was called
        mock_property_service.validate_property_values.assert_called_once_with(
            1, 
            {'material': 'Polyester', 'color': 'Blue', 'custom_notes': 'Updated notes'}
        )
        
        # Verify properties were updated
        assert mock_component_instance.properties == {'material': 'Polyester', 'color': 'Blue', 'custom_notes': 'Updated notes'}
        assert 'properties' in changes
        assert changes['properties']['old'] == {'material': 'Cotton', 'color': 'Red'}
        assert changes['properties']['new'] == {'material': 'Polyester', 'color': 'Blue', 'custom_notes': 'Updated notes'}


@patch('app.services.component_service.PropertyService')
def test_should_fail_update_when_property_validation_fails(mock_property_service, component_service, app_context):
    """
    Test: Component update fails with invalid property data
    Given: Existing component and invalid property updates
    When: update_component is called with invalid properties
    Then: ValueError should be raised with property validation errors
    """
    with patch('app.services.component_service.Component') as mock_component:
        ...
        with pytest.raises(ValueError) as exc_info:
            component_service_instance._update_basic_fields(mock_component_instance, update_data)
...
@patch('app.services.component_service.PropertyService')
def test_should_return_component_type_properties_configuration(mock_property_service, app_context):
    """
    Test: Get component type properties configuration
    Given: Component type with defined properties
    When: get_component_type_properties is called
    Then: Property configuration should be returned
    """
    # Mock property service response
    mock_property_service.get_property_form_config.return_value = {
        'material': {
            'display_name': 'Material',
            'data_type': 'select',
            'required': True,
            'options': [{'id': 1, 'name': 'Cotton'}, {'id': 2, 'name': 'Polyester'}],
            'placeholder': 'Material type of the component',
            'display_order': 1
        },
        'color': {
            'display_name': 'Color',
            ...
            'placeholder': 'Color of the component',
            'display_order': 2
        }
    }
    
    result = ComponentService.get_component_type_properties(1)
    
    # Verify property service was called
    mock_property_service.get_property_form_config.assert_called_once_with(1)
    ...
    assert result['material']['data_type'] == 'select'
    assert result['material']['required'] == True
    assert len(result['material']['options']) == 2


@patch('app.services.component_service.PropertyService')
def test_should_validate_component_properties_correctly(mock_property_service, app_context):
    """
    Test: Component property validation
    Given: Component type and property values
    When: validate_component_properties is called
    ...
    result = ComponentService.validate_component_properties(1, properties_data)
    
    # Verify property service was called
    mock_property_service.validate_property_values.assert_called_once_with(1, properties_data)
...
@patch('app.services.component_service.PropertyService')
def test_should_process_component_properties_from_form_data(mock_property_service, component_service, app_context):
    """
    Test: Process component properties from form data
    Given: Component type and form data with properties
    When: process_component_properties is called
    Then: Properties should be extracted and processed correctly
    """
    # Mock property service response
    mock_property_service.get_property_form_config.return_value = {
        'material': {
        ...
        'style': {
            ...
            'required': False,
            'options': [{'id': 1, 'name': 'Casual'}, {'id': 2, 'name': 'Formal'}]
        }
    }
    ...
    form_data = {
        'material': 'Cotton',
        'style': 'Casual, Formal',
        'custom_properties': '{"thread_count": 200, "special_treatment": "waterproof"}',
        'other_field': 'ignored'
    }
    
    result = component_service.process_component_properties(1, form_data)
...
@patch('app.services.component_service.PropertyService')
def test_should_include_property_config_in_component_data(mock_property_service, component_service, app_context):
    """
    Test: Component data includes property configuration
    Given: Component with properties and component type
    When: _build_component_data is called
    Then: Component data should include property configuration
    """
    # Mock property service response
    mock_property_service.get_property_form_config.return_value = {
        'material': {
    ...
    mock_component = Mock()
    mock_component.id = 1
    mock_component.product_number = 'SHIRT-001'
    mock_component.description = 'Test component'
    mock_component.component_type_id = 1
    mock_component.component_type = Mock(id=1, name='T-Shirt')
    mock_component.supplier = Mock(id=1, supplier_code='SUP001')
    mock_component.properties = {'material': 'Cotton', 'color': 'Red'}
    mock_component.created_at = None
    mock_component.updated_at = None
    mock_component.proto_status = 'pending'
    mock_component.proto_comment = None
    ...
    mock_component.brands = []
    mock_component.categories = []
    mock_component.keywords = []
    
    result = component_service._build_component_data(mock_component)
    
    # Verify property service was called
    mock_property_service.get_property_form_config.assert_called_once_with(1)
    
    # Verify result includes property configuration
    assert 'property_config' in result
    assert 'material' in result['property_config']
    assert result['property_config']['material']['display_name'] == 'Material'
    assert result['properties'] == {'material': 'Cotton', 'color': 'Red'}